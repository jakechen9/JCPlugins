...
RhythmicCreatorAudioProcessor::~RhythmicCreatorAudioProcessor() = default;
...
void RhythmicCreatorAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    mAudioFilterL.processBlock(buffer.getWritePointer(0), buffer.getNumSamples());
    mAudioFilterR.processBlock(buffer.getWritePointer(1), buffer.getNumSamples());
